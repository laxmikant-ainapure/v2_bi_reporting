...
def generate_scan_details_report(bi_connection):
    ...
    try:        
        ...
        if scan_cursor is not None:
            ...
            'from (select email,array_to_string(array_agg(distinct languages),'"','"') as lang,sum(repocount) as repocount,sum(scancount) as scancount,'
            ' sum(loc) as loc,max(signed_up_date) as signed_up_date,max(account_type)account_type,max(last_login_dt) as "Last Login Date" ,max(user_status) as user_status,max(package_name) as package_name,(license_expiry_date)as license_expiry_date from ( select u.email,lm.value as loc,array_to_string( array_agg(distinct l."name"),'"','"') as languages,max(u.created_dt) as signed_up_date,count(distinct r.id) as repocount, '
            ' count(rsh.id) as scancount ,max(o."type") as account_type,max(ui.last_login_dt) as last_login_dt , (case when max(ou.status) ='"'D'"' then '"'Deleted'"' else '"'Active'"' end) as  user_status,max(o.package_name) as package_name,(o.license_expiry_date) as license_expiry_date  from repository r '
            ...
            ' group by 1,2,license_expiry_date)s group by 1,license_expiry_date)s order by 2;')  
